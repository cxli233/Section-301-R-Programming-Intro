---
title: "Session 9 in-class activity"
author: "Chenxin Li"
date: "2026-06-18"
...
# Introduction
In this lesson, we will explore the best practice for means separation visualizations. 
Means separation is one of the most common graph types in scientific publications. 
...
2. What are the optimal graph types given the number of observations? 
3. How do you handle means separation plots in multifactorial experiments? 
...
For the first example, let's simulate some data and make a plain old bar plot. 
...
## Bar plot (do not recommend for mean separation) 
...
In a box plot, the center line is the median. 
...
## Showing all data points (even better)
For means separation plots, my go-to is plotting all dots with lateral offsets based on density. 
...
The name comes from the spread of dots looking like a swarm of bees.
...
# What are the optimal graph types given the number of observations? 
Be very careful with data that has a low number of observations (n < 30), and very large number of observations (n > 100). 
...
```{r}
tissue_culture <- read_excel("../Data/S9_example1.xlsx")
head(tissue_culture)
```
...
They have 19 cultivars of the plant, 3 explant types, and 3 hormone treatments. 
...
The value after `~` is the column name to make subplots from, which is "Variety" in the table. 
...
Can you explain in what scenario is it more appropriate to have `scales = "free"` turned on vs off?  

## Q2
Load "S9_activity1.csv" from the Data folder and view the first 6 rows. 
This is an experiment on oats. 
The experiment looked at the yield of oats across: 

* 4 fertilizer treatments
* 4 varieties 

The experiment was repeated across 4 blocks (sections in an experimental field). 

Make a mean separation visualization to compare the effect of fertilizer across varieties.  
Hints: 

1. Block, fertilizer, and variety data were stored as numeric variable. Change it to categorical data (characters). 
2. Is this a multi-factorial experiment? 
3. Consider the use of facets when appropriate. 
